...
==Issues
  ...
  =General
    ...
    - deleting a craft caused an interface crash for a short while (i think until System had processed)
    - attempting to restore to the version where a craft was deleted results in an interface crash (but no changes to state).


==Planned/Todo
  * quicksave and persistent tracking - inprogress
    - ability to write commit messages for saves
